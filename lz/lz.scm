      ...
      (when (ntp>? future (nz-cur-t nz))
	    ...
	    (let ((t (lz-tick (nz-lz nz)))
	      ...
	      (cond
	       ...
	       ((char=? t #\e) (play (nz-cur-t nz) (nz-sound nz 4 v (* (nz-tk nz) (nz-bpm-mult nz))) 0.2))
	       ((char=? t #\f) (play (nz-cur-t nz) (nz-sound nz 5 v (* (nz-tk nz) (nz-bpm-mult nz))) -0.2))
	       ((char=? t #\g) (play (nz-cur-t nz) (nz-sound nz 6 v (* (nz-tk nz) (nz-bpm-mult nz))) 0.7))
	       ((char=? t #\h) (play (nz-cur-t nz) (nz-sound nz 7 v (* (nz-tk nz) (nz-bpm-mult nz))) -0.7))
	       ;;((char=? t #\[) (nz-dup! nz))
	       ;;((char=? t #\]) (nz-pop! nz))
	       )
   ...
   (list
    (list 
     ...
     (lambda (v d) (mul (adsr 0 d 0 0)
			(white (mul 10 (adsr 0 d 0 0)))))
     (lambda (v d) (mul (adsr 0 d 0.3 (* d 5))
			(white (mul 300 (pow (adsr 0 d 0.9 (* d 4)) 10)))))
     (lambda (v d) (mul (adsr 0 d 0 0)
			(sine (mul 100 (adsr 0 d 0 0)))))
     )

    (list ;; snappy
     (lambda (v d) (moogbp (mul (adsr 0 d 0 0) (white 12)) 
			   (adsr 0 d 0.5 0.5) 0.1))
     (lambda (v d) (moogbp (mul (adsr 0 d 0 0) (pink 13)) 
			   (adsr 0 d 0.2 0.1) 0.3))
     (lambda (v d) (crush (moogbp (mul (adsr 0 d 0.1 0.5) (white 100)) 
				  (adsr 0 d 0.2 0.1) 0.5)
			  (* (modulo v 41) 0.1) 0.3))
     (lambda (v d) (mul (adsr 0 d 0 0)
